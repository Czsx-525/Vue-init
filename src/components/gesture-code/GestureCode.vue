...
export default {
  ...
  mounted () {
    this.drawColor = this.successColor
  ...
  methods: {
    ...
    drawCircle (x, y) {
      // 初始化解锁密码面板 小圆圈
      this.ctx.strokeStyle = this.drawColor // 密码的点点默认的颜色
    ...
    createCircle () {
      ...
      var count = 0
      this.radius = this.ctx.canvas.width / (4 * n) // 公式计算
      this.lastPoint = []
      this.circleArr = []
      this.restPoint = []
      var r = this.radius
    ...
    onEndHandler (e) {
      if (this.touchedFlag) {
        ...
        if (this.lastPoint.length < 4) {
          Toast.fail(this.$t('gestureLogin.minPoint'))
          this.onReset()
          return
        }
    ...
    onUpdateHandler (position) {
      ...
      this.drawPoint(this.drawColor) // 每帧花轨迹
      this.drawStatusPoint(this.drawColor) // 每帧花轨迹

      this.drawLine(this.drawColor, position, this.lastPoint) // 每帧画圆心
    ...
    errorCode () {
      this.drawColor = this.errorColor
      setTimeout(() => {
        this.onReset()
      }, 1500)
    }
