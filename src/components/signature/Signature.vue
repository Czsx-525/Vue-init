...
export default {
  ...
  data () {
    return {
      value: null, // base64
      domId: `signature-${Math.random().toString(36).substr(2)}`, // 生成唯一dom标识
  ...
  methods: {
    init () {
      ...
      this.dpr = typeof window !== 'undefined' && this.config.isDpr
    ...
    setLineWidth () {
      ...
      let returnNum =
        ...
        ((this.config.maxWriteWidth - this.config.minWriteWidth) * diffTime) / 30
